...
- **In the paradigm:** models reason the way they generate — statistically. They *author* rules well and *execute* long exact derivations badly, and no amount of test-time compute changes that: [a model is not a calculator](/blog/a-model-is-not-a-calculator/), and deduction is calculation over a different algebra. The industry has already conceded the point for arithmetic — every serious agent stack hands the model a calculator. Logos extends the same concession to logic: [Deem](/deem/introduction/) is not a bundled query library but the language's **declarative plane** — the offload target for reasoning itself. The model states facts and rules; the engine runs the exact fixpoint. The evidence is already public: on logic benchmarks where frontier models collapse under growing search spaces — and scaling inference-time compute [does not lift the curse](https://arxiv.org/abs/2502.01100) — formalize-then-solve architectures gain [sixty-five points, absolute](https://arxiv.org/abs/2502.15776). That is not an optimization; it is a different regime.

The deeper payoff of the declarative plane is *feedback quality*. An imperative program is a black box: the distance between a mistake and its symptom is unbounded, and reconstructing the "why" is expensive expert work — entire industries (tracing, observability, time-travel debugging) exist to retrofit weak receipts onto executions that kept none. A reasoner is the opposite kind of artifact: every conclusion carries its justification, every rejection its reason. In Logos this is not an add-on — Deem's incremental engine tracks provenance through change, and compile-time reasoners are arriving in the metaprogramming layer, where a rejected declaration fails *with the derivation of why*: which fact is missing, by which rule. For an agentic loop, that is the difference between a reward signal and an error code — the model's own error-cause abduction, which is pattern matching, finally gets chains short enough to land on.

The bet, stated plainly: as models come to write most code, languages begin to compete on the quality of the feedback they give the model — and the winning designs will be strong in *both* components at once, imperative and declarative, with reasoners doing for inference what the ALU does for arithmetic. Logos is built as exactly that pair.
