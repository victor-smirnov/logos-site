// API search over the [path, kind] index. Wires EVERY widget marked
// [data-search="api"] — the desktop sidebar and the mobile drawer each have
// one — so search works in both. Index is fetched lazily and shared.
(function () {
  'use strict';
  ...
  function nameOf(path) {
    var parts = path.split('::');
    if (parts[1]) { var top = parts[0]; return top.slice(top.lastIndexOf('.') + 1) + '::' + parts[1]; }
    return path.slice(path.lastIndexOf('.') + 1);
  }
  function esc(s) { return s.replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;'); }

  // Rank: exact leaf, leaf prefix, leaf/display substring, then full-path.
  ...
  function setup(box) {
    ...
    var list = box.querySelector('.api-search-results');
    ...
    function search(q) {
      ...
      load().then(function (idx) {
        var scored = [];
        for (var i = 0; i < idx.length; i++) { var s = score(idx[i][0], q); if (s >= 0) scored.push([s, idx[i]]); }
  ...
  document.querySelectorAll('.api-search[data-search="api"]').forEach(setup);
