...
Scope: package declarations, `use` imports, visibility, the prelude, module manifests, exports, loading, symbol identity, and cross-module linkage. Rules are extracted from the compiler's grammar, sema (collect / module-loader / manifest), and codegen layers; each `### ` heading carries the rule's permanent id.
...
A compilation unit begins with zero or more inner doc-comments (`//!`, `/*! */`) and inner attributes (`#![...]`), then `package NAME ('.' IDENT)* ';'`. The dotted path names the package to arbitrary depth (first component = NAME, remaining = PATH_PARTS). After the package line come zero-or-more use-declarations, then zero-or-more items.
...
**Divergence from Rust:** Rust has no `package` header; module identity comes from the file path plus `mod` items. Logos requires an explicit `package a.b.c;` line naming the full dotted package path.


_Source: `tools/peg_gen_cpp/grammars/logos.peg#L495-L496`_
...
_Source: `src/compiler/module_loader.cpp#L403-L455`_
...
_Source: `src/compiler/sema_expr.cpp#L2760-L2803`_
...
_Source: `src/compiler/sema_expr.cpp#L2805-L2814`_



## Item declarations

### `module.item.pub-module-linkage-visibility` — `pub(module)` vs plain `pub` visibility

An item declaration's visibility is carried by a VIS sub-node: NAME="module" denotes `pub(module)` (module-linkage visibility); an absent NAME denotes plain `pub`.


_Source: `tools/peg_gen_cpp/grammars/logos.peg#L84`_
...
_Source: `src/compiler/module_loader.cpp#L1254-L1256`, `src/compiler/module_loader.cpp#L1217-L1219`_



## Visibility (`pub` / `pub(module)`)

### `module.vis.pub-module` — `pub(module)` module-linkage visibility

Item visibility is `pub(module)` or plain `pub`. `pub(module)` grants module-linkage: visible to other packages within the same module but not exported to external consumers; plain `pub` is fully exported. `module` is a contextual keyword (matched as a bare IDENT, validated == "module" in sema).


_Source: `tools/peg_gen_cpp/grammars/logos.peg#L1336-L1344`_
...
_Source: `src/compiler/sema_collect.cpp#L3913-L3917`, `src/compiler/sema_collect.cpp#L4027-L4031`, `src/compiler/sema_collect.cpp#L4123-L4127`, `src/compiler/sema_collect.cpp#L4193-L4197`_
...
_Source: `src/compiler/sema_impl.hpp#L3157-L3175`_
...
_Source: `src/compiler/sema_collect.cpp#L751-L759`, `src/compiler/sema_impl.hpp#L3119-L3129`_
...
_Source: `src/compiler/sema_impl.hpp#L2428`, `src/compiler/sema_impl.hpp#L2548-L2551`_
...
_Source: `src/compiler/sema_impl.hpp#L3143-L3153`_
...
_Source: `src/compiler/sema_impl.hpp#L3202-L3209`_



## Module attributes

### `module.attr.inner-vs-item-attribute` — #![attr] is a distinct module/file-level inner attribute

`#![name]` / `#![name(args)]` / `#![name=val]` (INNER_ANNOTATION) is file/module-scoped, structurally identical in payload to a per-item ANNOTATION but tagged with a distinct CODE so sema can separate module-level attributes (e.g. `#![no_implicit_prelude]`) from item-level ones.


_Source: `tools/peg_gen_cpp/grammars/logos.peg#L311`_
...
_Source: `src/compiler/sema.cpp#L7441-L7457`, `src/compiler/sema.cpp#L8059-L8067`_
...
**Divergence from Rust:** note — Logos path model uses `.` for packages, `::` for items.
...
_Source: `src/compiler/module_loader.cpp#L135-L158`_
...
**Divergence from Rust:** no Rust analog; disambiguates when more than one module could supply the same dotted path.


_Source: `tools/peg_gen_cpp/grammars/logos.peg#L500-L505`, `tools/peg_gen_cpp/grammars/logos.peg#L519-L527`_
...
_Source: `src/compiler/sema_impl.hpp#L1093-L1110`_
...
**Divergence from Rust:** note — part of Logos's C++-style module-linkage system; no direct Rust equivalent.
...
**Divergence from Rust:** Logos addition: per-import module qualification (no Rust equivalent).


_Source: `src/compiler/sema_collect.cpp#L192-L225`_
...
_Source: `src/compiler/module_loader.cpp#L159-L196`_


### `module.use.path` — Plain use declaration

`[pub] use pkg ('.' IDENT)* ';'` brings a dotted package path into scope; `pub use` re-exports it. Path components after the head use a leading-dot separator (`path_dot_ident <- '.' IDENT`, with a negative lookahead against a following `.{` so the last component before a variant-shorthand brace-list is left for that alternative).
...
_Source: `tools/peg_gen_cpp/grammars/logos.peg#L504-L522`, `tools/peg_gen_cpp/grammars/logos.peg#L529-L533`_
...
_Source: `src/compiler/module_loader.hpp#L126-L127`, `src/compiler/module_loader.hpp#L144-L145`_
...
_Source: `src/compiler/sema_impl.hpp#L1111-L1115`_


### `module.use.variant-shorthand` — Enum-variant bare-name import

`use pkg.Path.Type.{V1, V2, ...};` brings the named variants of enum `Type` into bare (unqualified) scope; trailing comma allowed. The last dotted component before `.{...}` is the enum type name.
...
_Source: `tools/peg_gen_cpp/grammars/logos.peg#L509-L513`, `tools/peg_gen_cpp/grammars/logos.peg#L529-L533`_


### `module.use.variant-shorthand-vs-subpackage` — use {..} last-segment case disambiguates variant vs sub-package

`use pkg.Path.X.{V1, V2, …};` (USE_VARIANTS) disambiguates the grouped braces by the case of the last dotted segment (TYPE_NAME): an uppercase first character means enum-variant bare-name shorthand (X is an enum, V1/V2 its variants); a lowercase first character means a grouped sub-package import (X is a package, the braces list sub-names).


_Source: `tools/peg_gen_cpp/grammars/logos.peg#L310`_
...
**Divergence from Rust:** Disambiguation by identifier capitalization is a Logos convention, not a Rust rule.


_Source: `src/compiler/module_loader.cpp#L167-L204`_


### `module.use.variants-braced-list` — `use pkg.Path.Type.{V1, V2, …};` imports selected enum variants

A USE_VARIANTS import form `use pkg.Path.Type.{V1, V2, …};` binds the listed variant names of enum Type into scope; its AST carries TYPE_NAME (the enum type reference) and VARIANTS (an array of variant-name sub-nodes).


_Source: `tools/peg_gen_cpp/grammars/logos.peg#L82-L83`_
...
**Divergence from Rust:** Logos addition (`from <module>` import selector); no Rust equivalent.


_Source: `src/compiler/module_loader.cpp#L1594-L1611`, `src/compiler/module_loader.cpp#L1280-L1282`_
...
_Source: `src/compiler/module_loader.cpp#L1603-L1611`_
...
_Source: `src/compiler/sema_impl.hpp#L3130-L3142`_
...
_Source: `src/compiler/sema_impl.hpp#L2862-L2864`, `src/compiler/sema_impl.hpp#L3083-L3109`_
...
_Source: `src/compiler/module_loader.hpp#L135-L141`_
...
_Source: `src/compiler/module_loader.hpp#L131-L134`_
...
**Divergence from Rust:** Logos addition: implicit prelude is prefix-scoped to the lang tier (transitional; manifest-tier system intended).


**Uncertainty:** Exact prefix set is a transitional heuristic per source comments.


_Source: `src/compiler/module_loader.cpp#L1397-L1432`, `src/compiler/module_loader.cpp#L1566-L1571`_
...
**Divergence from Rust:** Logos uses a named prelude *package*; the model parallels Rust's std prelude but is package-granular.


_Source: `src/compiler/module_loader.cpp#L95-L103`_
...
_Source: `src/compiler/sema_collect.cpp#L240-L266`_
...
_Source: `src/compiler/sema_impl.hpp#L786-L792`_
...
_Source: `src/compiler/module_loader.cpp#L1566-L1571`, `src/compiler/module_loader.cpp#L1555-L1565`_
...
_Source: `src/compiler/module_loader.cpp#L57-L78`, `src/compiler/module_loader.cpp#L96`_
...
_Source: `src/compiler/sema_collect.cpp#L4384-L4406`_
...
_Source: `src/compiler/sema_impl.hpp#L3120-L3176`_
...
_Source: `src/compiler/module_loader.cpp#L1613-L1641`_
...
_Source: `src/compiler/sema_impl.hpp#L3732-L3746`_
...
_Source: `src/compiler/module_manifest.cpp#L26-L27`_
...
_Source: `src/compiler/module_manifest.cpp#L35-L65`_
...
_Source: `src/compiler/module_manifest.cpp#L24-L33`_
...
_Source: `src/compiler/module_manifest.cpp#L42-L49`_
...
_Source: `src/compiler/module_manifest.cpp#L59-L65`, `src/compiler/module_manifest.hpp#L62-L66`_
...
_Source: `src/compiler/module_manifest.cpp#L69-L70`_
...
_Source: `src/compiler/module_manifest.cpp#L50-L58`, `src/compiler/module_manifest.hpp#L52-L60`_
...
_Source: `src/compiler/module_manifest.cpp#L66`_
...
_Source: `src/compiler/module_manifest.cpp#L71`_
...
_Source: `src/compiler/module_loader.cpp#L213-L244`, `src/compiler/module_loader.cpp#L378-L388`_
...
_Source: `src/compiler/module_loader.cpp#L228-L233`_
...
_Source: `src/compiler/module_loader.hpp#L79-L86`_
...
_Source: `src/compiler/module_loader.hpp#L87-L93`_
...
_Source: `src/compiler/module_loader.hpp#L106-L112`_
...
_Source: `src/compiler/module_loader.hpp#L40-L45`, `src/compiler/module_loader.hpp#L73-L78`_



## Extern / foreign modules

### `module.extern.block-form` — `extern` block groups same-ABI FFI items

`[unsafe] extern ["ABI"] { extern_block_item* }` groups multiple extern items under one shared ABI (the block form defaults to Logos-internal ABI when the string is omitted). The `unsafe` marker on the block is accepted syntactically but carries no additional semantics in this slice (item-level `safe fn`/`safe static` refinements are a stated follow-up).


_Source: `tools/peg_gen_cpp/grammars/logos.peg#L1269-L1287`_


### `module.extern.fn-decl` — Standalone `extern` function declaration

`extern ["ABI"] fn NAME(params[, ...]) [-> Ret];` declares an FFI function with a per-declaration ABI string; a trailing `, ...` after the parameter list marks it variadic; the return type is optional. Validation of the ABI string against the supported set ("C", "C-unwind", "system", "Rust") happens in sema, not in the grammar.


_Source: `tools/peg_gen_cpp/grammars/logos.peg#L1269-L1276`, `tools/peg_gen_cpp/grammars/logos.peg#L1304-L1315`_


### `module.extern.item-form` — Items inside an `extern` block use bare syntax

Inside `extern ["ABI"] { ... }`, function items use bare `fn NAME(params[, ...]) [-> Ret];` (no redundant `extern` keyword; ABI is inherited from the enclosing block at splice time), and static items use `static [mut] NAME: TypeRef;` with no initializer, marking it an external (imported) global.


_Source: `tools/peg_gen_cpp/grammars/logos.peg#L1288-L1303`_
...
_Source: `src/compiler/module_loader.hpp#L126-L146`, `src/compiler/module_loader.hpp#L142`_
...
_Source: `src/compiler/module_loader.cpp#L1598-L1599`, `src/compiler/module_loader.cpp#L1619-L1620`, `src/compiler/module_loader.cpp#L1583-L1584`_
...
_Source: `src/compiler/module_loader.cpp#L1626-L1627`, `src/compiler/module_loader.cpp#L1664-L1665`, `src/compiler/module_loader.cpp#L1673-L1679`_
...
_Source: `src/compiler/sema_impl.hpp#L1083-L1095`_
...
_Source: `src/compiler/mlir_gen_fn.cpp#L238-L242`, `src/compiler/mlir_gen_fn.cpp#L260`, `src/compiler/mlir_gen_fn.cpp#L271-L282`_
...
_Source: `src/compiler/module_manifest.cpp#L85-L98`, `src/compiler/module_manifest.hpp#L73-L79`_
...
_Source: `src/compiler/module_manifest.cpp#L76-L83`, `src/compiler/module_manifest.cpp#L88`_
...
_Source: `src/compiler/module_loader.cpp#L1533-L1548`, `src/compiler/module_loader.cpp#L1063-L1067`_
...
_Source: `src/compiler/module_loader.hpp#L23-L30`_
...
_Source: `src/compiler/module_loader.hpp#L19`, `src/compiler/module_loader.hpp#L17-L19`_
...
**Divergence from Rust:** Logos addition: semantic-version ABI gate on binary modules (Rust has no stable cross-version library ABI).


_Source: `src/compiler/module_loader.cpp#L1100-L1144`, `src/compiler/module_loader.cpp#L1182-L1184`_
...
_Source: `src/compiler/module_loader.cpp#L1100-L1113`, `src/compiler/module_loader.cpp#L1130-L1142`, `src/compiler/module_loader.cpp#L1110-L1110`_
...
_Source: `src/compiler/module_loader.hpp#L21`, `src/compiler/module_loader.hpp#L24-L27`_
...
_Source: `src/compiler/module_loader.hpp#L32-L37`_



## Cross-module statics
...
_Source: `src/compiler/mlir_gen_dyn.cpp#L668-L681`, `src/compiler/mlir_gen_dyn.cpp#L694-L701`, `src/compiler/mlir_gen_dyn.cpp#L716-L723`_
...
_Source: `src/compiler/mlir_gen_types.cpp#L79-L93`, `src/compiler/mlir_gen_types.cpp#L483-L488`, `src/compiler/mlir_gen_types.cpp#L588-L589`_
...
**Divergence from Rust:** Module/separate-compilation model; no direct Rust analogue.


_Source: `src/compiler/mlir_gen_dyn.cpp#L219-L292`, `src/compiler/mlir_gen_dyn.cpp#L358-L368`, `src/compiler/mlir_gen_dyn.cpp#L394-L396`_
...
_Source: `src/compiler/sema.cpp#L1404-L1415`, `src/compiler/sema.cpp#L1460-L1463`_
